...
- port4 は **送信(OUT)のみ・応答(IN)なし** → センサではなく **基板上のデバッグ UART へのテキスト出力**。
- ヘッダ `04 0a 00` の後に ASCII。復元すると脚ごとの状態ダンプ:

```
FR;  0 0 0;  Err 0 0 0;  Tem 44 37 38;  Pos -0.125 1.319 ...
```

- `Err`=エラーフラグ, `Tem`=3モータ温度(℃), `Pos`=関節角(rad)。
