...
### 動作確認済みベースライン設定
```
cargo run -p go2-gait-runner -- run eth0 models/unitree_go2/go2.misa \
    0.02   3     4       200  6    0.04     1.667    0.85          on       1.73
  # vx  inplace forward  kp   kd  swing_h  cycle_s  four_support  grav_ff  ff_scale
```
...
- misarta の Go2 モデルは合計 ~8.7kg、実機は ~15kg。FF は荷重を全支持脚へ
  `fz = weight/n_stance` で**均等配分**し、各支持脚で `tau = -Jᵀ·f`（`grav_ff on`）。
- `ff_scale`（既定 1.0）で配分重量をスケールし実機質量へ合わせる（~1.73x）。
- 効果（kp=200, calf 平均追従誤差 |cmd-act|）: **FF無 0.038 → FF×1.0 0.024 → FF×1.73 0.014**（≈0.8°）。
  脚の沈み込みをほぼ解消。

### 既知の残課題
- **揺れ（左右の重心移動）は LinearCrawl の原理由来**。静的安定歩容は遊脚を上げる前に
  重心を残り3脚の支持三角形内へ寄せる必要があり、この移動が「揺れ」として見える。
  ゲイン/FF では消せない。軽減は stance を広げる/重心を下げる/weight shift を緩める方向で可能。
- **後脚 hip の追従誤差が前脚の約3倍**（RR/RL ~0.034 vs FR/FL ~0.010、2回とも再現）。
  FF が荷重を**均等配分**し重心位置（中央〜やや前寄り）を考慮していないため、後脚 hip ロールの
  踏ん張り負荷を補償しきれていない。→ FF を重心位置に応じた前後配分へ改良予定。
